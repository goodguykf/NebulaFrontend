import { RailAnalysisView } from "@/components/rail/RailAnalysisView";
...
export default function RailAnalysisScreen() {
  ...
  return (
    ...
    >
      {loading ? <LoadingState message="Loading rail analysis…" /> : null}

      {error ? (
        <ErrorState
          title="Unable to open analysis"
          message="We couldn't load this rail result."
          detail={error}
          onRetry={reload}
        />
      ) : null}

      {!loading && !error && record?.subsystem === "rail" ? (
        <View style={styles.stack}>
          ...
          {record.result ? (
            <RailAnalysisView result={record.result} />
          ) : (
            <ErrorState
              ...
              message={
                record.status === "failed"
                  ...
                  : "This job has not returned a rail classification."
              }
            />
          )}
        </View>
      ...
      {!loading && !error && record && record.subsystem !== "rail" ? (
        <ErrorState
          title="Unexpected subsystem"
          message="This record is not a rail analysis."
        />
      ) : null}
    </ScreenContainer>
  );
